...
static auto place = DeviceKind::GPU;
...
protected:
    ...
    void SetUp() override {
        filename_ = "test_gpu_support.csv";
    }
...
TEST_F(CsvTest, ExportCPUSupportCSVMatchesRegistry) {
    // Export CSV
    export_support_csv(filename_, place);
    ...
    for (const auto& op_name : operators) {
        ...
        for (const auto& pair : dtype_col) {
            ...
            bool expected = is_kernel_registered(op_name, place, dtype);
